...
class NCSNRunner():
    ...
    def train(self):
        ...
        early_end = False
        train_logger = TensorBoardLogger("../results", 'train')
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for batch, (X, y) in enumerate(dataloader):
                ...
                # Loss
                itr_start = time.time()
                #print(X.shape); print(cond.shape)      #WIP
                
                #print(X.shape)
                #if cond is not None: print(cond.shape)
                #else: cond = torch.empty((1, 0, 64, 64)).to(X.device)
                #if cond_mask is not None: print(cond_mask.shape)
                #cond = X[:, 0, :, :]; X = X[:, 1::, :, :]
                ...
                # Optimize
                loss.backward()
                train_logger.experiment.add_scalar('L1 Loss', loss.item(), step)
    ...
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        for i, (real_, _) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)), desc="\nvideo_gen dataloader"):
            ...
            print(real.shape); print(pred.shape)
