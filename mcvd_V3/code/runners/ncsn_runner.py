...
class NCSNRunner():
    ...
    def train(self):
        ...
        #self.fid_metric = torchmetrics.image.fid.FrechetInceptionDistance(feature = 512,
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for batch, X in enumerate(dataloader):
                ...
                if step == 1 or step % self.config.training.val_freq == 0:
                    self.fid_metric = torchmetrics.image.fid.FrechetInceptionDistance(feature = 2048,
                                                        reset_real_features = True, normalize = True)
                    if random.random() <= 0.1:
                        for i in X.shape[2]:
                            self.fid_metric.update(torch.cat([X[0, :, i], X[0, :, i], X[0, :, i]],
                                                                dim = 0).unsqueeze(0), real = True)
                """
                ...
                if step == 1 or step % self.config.training.val_freq == 0:
                    ...
                    for i in range(test_X.shape[2]):
                        self.fid_metric.update(torch.cat([test_X[0, :, i], test_X[0, :, i], test_X[0, :, i]],
                                                                        dim = 0).unsqueeze(0), real = True)
                ...
                if (step % self.config.training.snapshot_freq == 0 or step % self.config.training.sample_freq == 0) and self.config.training.snapshot_sampling:
                    ...
                    if conditional:
                        ...
                        # Stretch out multiple frames horizontally
                        pred = stretch_image(pred, self.config.data.channels, self.config.data.image_size)
                        #pred_fid = pred.reshape((1, 1, self.config.data.image_size, self.config.data.image_size,
                        #                         self.config.data.num_frames)).permute(0, 1, 4, 2, 3)
                        #print(pred_fid.shape)     # [1, 1, 30, 64, 64]
                        #self.fid_metric.update(torch.cat([pred_fid[0, :, 15], pred_fid[0, :, 15], pred_fid[0, :, 15]],
                        #                                                            dim = 0).unsqueeze(0), real = False)
                        for i in [0, 7, 15, 22, 29]:
                            self.fid_metric.update(torch.cat([pred_fid[0, :, i], pred_fid[0, :, i], pred_fid[0, :, i]],
                                                                                    dim = 0).unsqueeze(0), real = False)

                        fid_result = torch.autograd.Variable(self.fid_metric.compute() / 1000, requires_grad = True)
                        self.fid_metric.reset(); del self.fid_metric
                        self.fid_metric = torchmetrics.image.fid.FrechetInceptionDistance(feature = 2048,
                                                        reset_real_features = True, normalize = True)
                        #self.fids.update(self.fid_metric.compute())
                        print(f"FID Metric: {fid_result.item() * 1000}")
                    ...
                    torch.save(pred, os.path.join(self.args.log_sample_path, 'samples_{}.pt'.format(step))); gif_save = []
                    #for i in range(pred_fid.shape[2]):
                    #    gif_save.append(Image.fromarray((pred_fid[0, 0, i, :, :].numpy() * 255.0).astype(np.uint8)))
                    #imageio.mimwrite(os.path.join(self.args.log_sample_path, f"video_grid_{step}.gif"), gif_save, fps = 4)
